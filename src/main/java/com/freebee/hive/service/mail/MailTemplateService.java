...
public class MailTemplateService {
    ...
    public String prepareTemplateModelSetup(final String mailTemplate, final Map<String, String> valueMap) {
        String content = mailTemplate;
        for (String key : valueMap.keySet()) {
            String placeholder = "${" + key + "}";
            String replacedValue = valueMap.get(key) != null ? valueMap.get(key) : "";
            if (content.contains(placeholder))
                content = content.replace(placeholder, replacedValue);
        }
        if (content.contains("${")) {
            content = content.replaceAll(".\\Q${\\E.*\\Q}\\E", "");
        }
        return content;
    }

    public String getEmailContent(String templateName, Map<String, String> model) throws IOException, TemplateException {
